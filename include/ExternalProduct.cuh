...
void BFVContext::external_product_T(uint64_tt* output, uint64_tt* cipher_modUp_QjtoT, uint64_tt* swk_modUp_RitoT, int l)
{
    int cipher_blockNum = ceil(double(l+1) / p_num);
    int blockNum = ceil(double(p_num+l+1) / gamma);

    // printf("l: %d , cipher_blockNum: %d , blockNum: %d\n" , l , cipher_blockNum , blockNum);
...
void BFVContext::external_product_T_swk_reuse(uint64_tt* output, uint64_tt* cipher_modUp_QjtoT, uint64_tt* swk_modUp_RitoT, int l, int batch_size)
{
    int cipher_blockNum = ceil(double(l+1) / p_num);
    int blockNum = ceil(double(p_num+l+1) / gamma);
...
void BFVContext::mult_PlaintextT(uint64_tt* cipher_modUp_QjtoT, PlaintextT& plain_T, int l)
{
    int blockNum = ceil(double(p_num+l+1) / gamma);
