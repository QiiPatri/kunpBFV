...
static void
INTT8pointPerThread_for_bfv_kernel1(uint64_tt *inout,
                           const uint64_tt *itwiddles,
                           const uint64_tt *itwiddles_shoup,
                           const int poly_num,
                           const int start_poly_idx,
                           ...
                           const uint64_tt modulus)
{
    ...
    for (int i = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        const uint64_tt *psi = itwiddles + n * twr_idx;
        const uint64_tt *psi_shoup = itwiddles_shoup + n * twr_idx;
        uint64_tt modulus_value = modulus;
        // uint64_tt modulus_value2 = pqt2_cons[twr_idx];
        uint64_tt modulus_value2 = modulus_value << 1;
        int n_init = 2 * m_idx * t + t_idx;
...
__global__ static void
INTT8pointPerThread_for_bfv_kernel2(uint64_tt *inout,
                           const uint64_tt *itwiddles,
                           const uint64_tt *itwiddles_shoup,
                           const uint64_tt *inv_degree_modulo,
                           const uint64_tt *inv_degree_modulo_shoup,
...
//intt_kernel
__global__ static void
INTT8pointPerThread_kernel1_for_QR(uint64_tt *inout,
                           const uint64_tt *itwiddles,
                           const uint64_tt *itwiddles_shoup,
                           const int poly_num,
                           const int start_poly_idx,
                           ...
                           const int poly_mod_len)
{
    ...
    for (int i = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        const uint64_tt *psi = itwiddles + n * twr_idx;
        const uint64_tt *psi_shoup = itwiddles_shoup + n * twr_idx;
        uint64_tt modulus_value = qr_cons[twr_idx];
        // uint64_tt modulus_value2 = pqt2_cons[twr_idx];
        uint64_tt modulus_value2 = modulus_value << 1;
        int n_init = 2 * m_idx * t + t_idx;
...
__global__ static void
INTT8pointPerThread_kernel2_for_QR(uint64_tt *inout,
                           const uint64_tt *itwiddles,
                           const uint64_tt *itwiddles_shoup,
                           const uint64_tt *inv_degree_modulo,
                           const uint64_tt *inv_degree_modulo_shoup,
...
//ntt_kernel
__global__ static void NTT8pointPerThread_kernel1_for_QR(uint64_tt *inout,
                           const uint64_tt *twiddles,
                           const uint64_tt *twiddles_shoup,
                           int poly_num,
                           int start_poly_idx,
    ...
    for (int tid = blockIdx.x * blockDim.x + threadIdx.x ;
        ...
        const uint64_tt *psi = twiddles + twr_idx * n;
        const uint64_tt *psi_shoup = twiddles_shoup + twr_idx * n;
        uint64_tt modulus = qr_cons[twr_idx];
        uint64_tt modulus2 = modulus << 1;
        int n_init = t / 4 / group * pad_idx + pad_tid + pad * (n_idx / (group * pad));
...
__global__ static void
NTT8pointPerThread_kernel2_for_QR(uint64_tt *inout,
                           const uint64_tt *twiddles,
                           const uint64_tt *twiddles_shoup,
                           int poly_num,
                           int start_poly_idx,
...
__launch_bounds__(128, 1)
static void
INTT8pointPerThread_for_test_kernel1(uint64_tt *inout,
                           const uint64_tt *itwiddles,
                           const uint64_tt *itwiddles_shoup,
                           const int poly_num,
                           const int start_poly_idx,
...
#pragma unroll
        for (int j = 0; j < 8; j++) {
            *(n1ntt_res + n_init + t / 4 * j) = samples[j];
        }
...
__global__ static void
INTT8pointPerThread_for_test_kernel2(uint64_tt *inout,
                           const uint64_tt *itwiddles,
                           const uint64_tt *itwiddles_shoup,
                           const uint64_tt *inv_degree_modulo,
                           const uint64_tt *inv_degree_modulo_shoup,
...
__global__ static void NTT8pointPerThread_for_test_kernel1(uint64_tt *inout,
                           const uint64_tt *twiddles,
                           const uint64_tt *twiddles_shoup,
                           int poly_num,
                           int start_poly_idx,
    ...
    for (int tid = blockIdx.x * blockDim.x + threadIdx.x ;
        ...
        for (int j = 0; j < 8; j++) {
            *(n1ntt_res + n_init + t / 4 * j) = buffer[pad_tid * (n1 + pad) + pad_idx + group * j];
        }
...
__launch_bounds__(128, 1)
static void
NTT8pointPerThread_for_test_kernel2(uint64_tt *inout,
                           const uint64_tt *twiddles,
                           const uint64_tt *twiddles_shoup,
                           int poly_num,
                           int start_poly_idx,
