...
public:
	...
	Key(uint64_t* a, uint64_t* b, int N, int L, int K, int dnum) : N(N), L(L), K(K), dnum(dnum)
	{
        cudaMalloc(&cipher_device, sizeof(uint64_tt) * N * (L + 1 + K) * dnum * 2);
        ax_device = cipher_device;
        bx_device = cipher_device + N * (L + 1 + K) * dnum;

        cudaMemcpy(ax_device, a, sizeof(uint64_tt) * N * (L + 1 + K) * dnum, cudaMemcpyHostToDevice);
        cudaMemcpy(bx_device, b, sizeof(uint64_tt) * N * (L + 1 + K) * dnum, cudaMemcpyHostToDevice);
	}
